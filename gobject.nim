...
template g_type_check_instance_cast*(instance, g_type, c_type: expr): expr = 
  var hhh = g_type
  (g_type_cic(instance, hhh, c_type))
...
proc g_object_new*(object_type: GType; first_property_name: cstring): GObject {.
...
#proc clear_object*(object_ptr: ptr GObject) {.importc: "g_clear_object", 
#    libgobj.}
proc clear_object*(object_ptr: var GObject) {.importc: "g_clear_object", 
    libgobj.}
...
#template g_define_type_extended_class_init(TypeName, type_name): string =
#  """
#  proc $2_class_intern_init(klass: gpointer) =
#    $2_parent_class = g_type_class_peek_parent (klass)
#    if $1_private_offset != 0:
#      g_type_class_adjust_private_offset(klass, addr $1_private_offset)
#    $2_class_init(cast[ptr $1Class](klass))
#
#""" % [TypeName, type_name]
...
macro no_g_define_type_extended*(tn, t, tp, f, c: static[string]): stmt =
  var cc = "\n" & c
...
proc $2_get_instance_private(self: $1): ptr $1Private =
  return cast[ptr $1Private](g_struct_member_p(self, $1_private_offset))
...
# f: GTypeFlags to pass to g_type_register_static()
# c: Custom code that gets inserted in the *_get_type() function.
macro xg_define_type_extended*(tn, t, tp, f, c: static[string]): stmt =
  var cc = "\n" & c
  cc = cc.splitLines.join("\n    ")
...
proc $2_get_instance_private(self: $1): $1Private =
  return cast[$1Private](g_struct_member_p(self, $1_private_offset))
...
# f: GTypeFlags to pass to g_type_register_static()
# c: Custom code that gets inserted in the *_get_type() function.
macro lg_define_type_extended*(tn, t, tp, f, c: static[string]): stmt =
  var cc = "\n" & c
  cc = cc.splitLines.join("\n    ")
...
# f: GTypeFlags to pass to g_type_register_static()
# c: Custom code that gets inserted in the *_get_type() function.
macro org_g_define_type_extended*(tn, t, tp, f, c: static[string]): stmt =
  var cc = "\n" & c
  cc = cc.splitLines.join("\n    ")
...
# f: GTypeFlags to pass to g_type_register_static()
# c: Custom code that gets inserted in the *_get_type() function.
macro nowin_g_define_type_extended*(tn, t, tp, f, c: static[string]): stmt =
  var cc = "\n" & c
  cc = cc.splitLines.join("\n    ")
...
# f: GTypeFlags to pass to g_type_register_static()
# c: Custom code that gets inserted in the *_get_type() function.
macro g_define_type_extended*(tn, tp, f, c: static[string]): stmt =
  var cc = "\n" & c
  cc = cc.splitLines.join("\n    ")
  var t = camel2snake(tn)
  var s = """

proc $2_init(self: $1) {.cdecl.}
proc $2_class_init(klass: $1Class) {.cdecl.}
var $2_parent_class: gpointer = nil
var $1_private_offset: gint
